...
配置 `apps/api/.env.example` 中的 `UPLOAD_TOKEN_SECRET`、全部 `OSS_*` 变量和 `PLATFORM_DOMAIN` 后启动 API。执行前先按 `docs/guides/DEPLOYMENT.md` 完成 bucket 权限、CORS、CDN 子域映射、泛域名 DNS、HTTPS 证书和地域合规检查。
