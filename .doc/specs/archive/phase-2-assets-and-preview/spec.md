...
- 受控云 OSS、公网 preview DNS/证书/CDN、云端部署 P95 与公网微信真机兼容性证明；这些属于部署基础设施启用验收，不再作为 Phase 2 软件能力退出条件；
...
2026-07-16 经负责人 `Pan` 确认，Phase 2 以本地/Mock 工程闭环验收：受控内存对象存储、20 次真实模板构建部署 P95、独立 loopback 静态 HTTP 服务读取 artifact、模板拨号/PDF 链接测试可作为本阶段退出证据。真实 OSS、公网 DNS/证书/CDN、云端 P95 和公网微信真机测试保留为部署基础设施启用门槛；在获得这些证据前，文档必须继续标为“阻塞/待确认”，不得把 Mock 结果表述为真实云验收。

## 验收标准

- [x] 金源素材清单覆盖 Logo、产品图、证书图、产品 PDF、微信二维码和厂房图；每项均有状态、来源/确认信息、复核要求和占位策略，未核验项明确标为待确认，内部预览替代方案由 `Pan` 批准。
- [x] AC-01～AC-03 回归通过：已登录运营人员创建站点后在同一事务中获得唯一 `siteId`、revision 1 与审计记录；非法配置准确定位字段；两个客户端基于同一 Revision 保存时仅第一个成功，第二个收到 `409 revision_conflict`。
- [x] 保存草稿 Revision 时，尚不存在的预留 Asset ID 可以保留；已存在但跨站或类型不匹配的 Asset 被拒绝。创建部署时，任何不存在、未复核、跨站、类型不匹配或未经批准的占位 Asset 都必须被拒绝。
- [x] 上传流程由服务端签发短时、限定对象键/Content-Type/大小的上传信息；本地受控适配器及 OSS 适配器契约均由服务端复核对象存在性、大小、MIME/文件特征、checksum 和归属后才创建不可变 Asset，客户端不接触长期密钥。
- [x] 后台能显示上传进度、复核状态、真实/占位标记及失败原因；占位素材须显示批准状态，未批准占位素材不能触发预览。
- [x] 指定有效 Revision 与模板版本可生成不可变 `BuildArtifact`；产物和资源置于 revision/artifact 专属路径，携带相同幂等键的重复部署请求只返回同一有效任务，不得覆盖或创建重复有效 artifact。
- [x] 本地/Mock 标准配置连续 20 次真实模板构建部署均为 `healthy`，最终复验 P95 为 6134.66 ms，不超过 10 分钟；独立 loopback 静态 HTTP 服务验证 artifact。真实 DNS/TLS/云端 P95 在基础设施启用时另行验收。
- [x] 未启动 API 网络监听器时，独立静态 HTTP 服务读取首页、产品、资质、关于、联系、构建 JS 与示例 Logo；模板自动化测试覆盖 `tel:` 拨号和可选 PDF 链接。公网微信真机兼容性在基础设施启用后另行验收。
- [x] 域名注册、DNS、证书、ICP/地域合规状态在部署指南中如实记录为“阻塞/待确认”；未满足前置条件时部署返回明确的不可发布状态，而非伪造真实 HTTPS 链接。
...
- [x] 后台交互测试覆盖 Revision 列表加载、选择历史 Revision、两次并发保存导致的 `409 revision_conflict`、提示当前服务端版本及重新加载最新版本后恢复编辑；不会静默覆盖本地编辑内容。
- [x] `pnpm acceptance:local`、`pnpm check`、`pnpm test`、`pnpm build` 通过；真实云 smoke 不属于本阶段退出条件。
...
- 本地性能与独立性：`pnpm acceptance:local` 连续执行 20 次真实模板构建，全部 `healthy`，最终复验 P95 6134.66 ms；独立 loopback 静态 HTTP 服务在未启动 API 网络监听器时覆盖 5 个路由、构建 JS 与示例 Logo；报告明确列出不得替代的外部证据。
- 素材：六类缺失素材的内部预览替代方案已由 `Pan` 于 2026-07-16 批准；真实客户素材继续标为待确认，未伪造证书、PDF 或二维码。
- 后续部署门槛：受控 OSS、平台 preview DNS/证书/CDN、真实 HTTPS smoke、云端 20 次 P95 与公网微信真机尚未执行；它们不阻塞 Phase 2 软件变更归档，也不得被标为真实云验收通过。
