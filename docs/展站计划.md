...
部署状态单独使用 `queued / building / deploying / retry_waiting / healthy / failed`；域名与备案为后续独立状态维度。
...
| **Phase 2：素材与预览** | 内部后台、受控上传适配器、异步构建、预览发布契约 | 以批准占位和本地/Mock 适配器完成 20 次部署、静态独立性及交互验收 | 已完成（真实云未启用） |
| **Phase 3：可靠性** | 日志、健康检查、幂等重试、artifact 回滚 | 部署失败不影响上一健康版本 | 已完成（本地/Mock 与 MySQL） |
| **V1 本地交付闭环** | ReviewRecord、内容状态、审核/反馈/确认、完整运营流程 | 创建到回滚全流程可追踪，MySQL 持久化与跨站隔离成立 | 已完成（本地/Mock 与 MySQL） |
...
- [x] 金源 Logo、PDF、产品图、证书、二维码与厂房图素材清单；真实文件继续待确认，内部预览替代方案已由 `Pan` 逐项批准
- [x] `pnpm acceptance:local` 连续 20 次真实模板构建部署全部健康；2026-07-17 复验 P95 3103.24 ms，独立 loopback 静态 HTTP 服务通过 active pointer 验证五个路由、构建 JS 与示例 Logo
- [x] Phase 3 本地可靠性：不可变 release、`site_preview_states` 原子指针、最多 3 次重试、阶段事件、同站回滚和 AC-09/10/12 隔离测试通过
- [x] Phase 3 MySQL 集成：追加 migration、复合外键、并发激活、lease fencing、事件顺序和跨站约束通过
- [x] V1 本地交付闭环：Revision `draft / review_requested / approved / archived` 状态、追加式 ReviewRecord、预览发送/客户反馈/客户确认、审核审计和后台时间线已实现
- [x] 本地运营端到端验收：创建 → 配置 → 发布 → 发送审核 → 反馈 → 修改 → 再发布 → 确认 → artifact 回滚通过；MySQL migration、重启持久化、复合归属和跨站拒绝通过
- [ ] 部署基础设施启用门槛：受控 OSS、平台 preview 域名、泛域名 DNS/证书、CDN、云端 P95 与公网微信真机（待配置，不阻塞 Phase 2 软件归档）
- [ ] `jinyuan-mvp` 的自动化测试与 CI（待补齐）

旧文档中“Demo 已完成”的表述已部分核验：模板和 Demo 可构建，但不等于真实云能力已启用。Phase 2 已按批准的本地/Mock 口径完成软件验收；真实素材、云基础设施与公网设备证据仍须在对外交付前补齐，不能用本地报告替代。`jinyuan-mvp` 的测试与 CI 继续作为 Phase 3 前遗留项。
...
1. **执行本地运营试跑** — 使用后台完成真实客户资料的创建、配置、预览、审核、修改与确认，采集人工工时和修改轮次
2. **闭环遗留素材** — 补齐或明确豁免 PDF、图片、二维码与金源真实素材
3. **决定是否进入 Phase 4** — 仅在需要公网交付时单独评审 OSS、平台域名、DNS、TLS、CDN 与备案
4. **继续后置 Agent** — 未获得稳定人工流程数据前不启动产品化 Agent
...
| v1.5 | 2026-07-16 | 回写 Phase 3 本地可靠性、MySQL 原子激活、重试、回滚与隔离验收 |
| v1.6 | 2026-07-17 | 新增并完成 V1 本地交付闭环，回写审核留痕、内容状态和完整运营流程验收 |
