...
代码已实现 OSS PUT 签名、服务端完成复核、不可变 Asset/Artifact 路径、异步 Deployment 状态、模板构建、独立轮询 Worker 和 HTTPS 健康检查。Deployment 与 Artifact 通过数据库租约领取，Worker 重启后可继续领取排队任务和过期租约。仓库中没有真实云凭据、平台域名、DNS、证书或备案证明，因此截至 2026-07-16 只能确认本地适配器和自动化测试，不能声称真实 HTTPS 预览已经交付。
