# Phase 3 预览部署与本地可靠性指南

## 当前状态

代码已实现不可变 Asset/Artifact/release、异步 Deployment、数据库租约、最多 3 次有界重试、阶段事件、原子 preview pointer 和同站 artifact 回滚。Worker 重启后可领取到期重试或过期租约，旧 lease token 和旧 pointer version 不能激活。Phase 3 已按本地/Mock 与 MySQL 口径完成软件验收；仓库中没有真实云凭据、平台域名、DNS、证书或备案证明，因此不能声称真实 HTTPS 预览已经交付。
