...
| v1.5 | 2026-07-17 | — | 回写 V1 本地交付闭环：内容状态、ReviewRecord、审核 API、MySQL 隔离与端到端验收 |
...
| `SiteRevision` | 不可变配置快照与正交审核状态 | siteId、revision、schemaVersion、config、contentStatus、createdBy |
...
| POST | `/sites/:id/rollbacks` | 将历史成功 artifact 原子切换为当前预览 |
| GET/POST | `/sites/:id/reviews` | 查询或新增预览发送、反馈与确认留痕 |
| POST | `/sites/:id/revisions/:revision/archive` | 以 expectedStatus 归档非当前 Revision |
| GET | `/sites/:id/audit-logs` | 查询本站追加式审计时间线 |
...
| **Phase 3：可靠性** | 日志、幂等重试、健康检查、artifact 回滚与隔离测试 | AC-09、AC-10、AC-12 及故障演练通过 | 已完成（本地/Mock 与 MySQL） |
| **V1 本地交付闭环** | ReviewRecord、内容状态、审核/反馈/确认与完整运营流程 | AC-13、本地端到端和 MySQL 持久化隔离通过 | 已完成（本地/Mock 与 MySQL） |
...
Phase 3 以不可变 `releases/{siteId}/{deploymentId}/{artifactId}`、数据库原子 preview pointer、最多 3 次白名单 transient 重试和追加式 Deployment 事件实现 AC-09/10/12。回滚创建新的 `kind=rollback` Deployment，不修改历史 artifact；跨站 artifact、Deployment、事件和回滚统一拒绝并写安全审计。本结论仅代表本地/Mock 与独立 MySQL 测试库验证，不解除 IA-01～IA-04。

V1 本地交付闭环为每个 Revision 持久化 `contentStatus`。`preview_sent` 必须绑定同站、同 Revision 的 healthy Deployment，并原子执行 `draft → review_requested`；`customer_feedback` 与 `customer_confirmed` 必须复用该次最近发送审核的 Deployment，分别执行 `review_requested → draft` 与 `review_requested → approved`。ReviewRecord 只追加，状态迁移携带 `expectedStatus`，记录与状态更新、审计在同一 MySQL 事务完成。非当前 Revision 可显式进入终态 `archived`。内容批准不等于正式发布，且不改变 Deployment 状态。
...
| AC-13 | 本地交付闭环 | 创建站点、配置、发布、发送审核、反馈、修改、再发布、确认和 artifact 回滚全流程可追踪；ReviewRecord、内容状态和 AuditLog 在 MySQL 持久化且跨站引用被拒绝 |
