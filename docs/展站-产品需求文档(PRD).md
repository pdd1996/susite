...
| **Phase 2：素材与预览** | 内部后台、OSS 直传、异步构建、HTTPS 预览 | AC-01～AC-08、AC-11 全部通过 | 待做 |
...
| AC-04 | 素材上传 | 非法 MIME、超限文件或跨 siteId 路径被拒绝；合法文件生成 Asset 记录 |
| AC-05 | 配置发布 | 指定 revision 发布后返回 jobId，重复幂等请求不产生重复有效任务 |
| AC-06 | 预览时效 | 连续 20 次标准配置部署中，P95 ≤ 10 分钟并获得统一格式 HTTPS URL |
| AC-07 | 健康检查 | 首页、产品、资质、关于、联系和关键资源全部成功后才进入 healthy |
| AC-08 | 微信与移动端 | 当期稳定版微信内置浏览器在 iOS 16+ 与 Android 12+ 的真机上均可打开、拨号、下载 PDF |
